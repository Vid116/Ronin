...
/**
 * @title RoninRumble1v1
 * @dev 1v1 contract for Ronin Rumble auto battler game
 * Handles 2-player matches with entry fees, match results, and prize distribution
 */
contract RoninRumble1v1 is ReentrancyGuard, Pausable, Ownable {
    // Constants
    uint256 public constant PLAYERS_PER_MATCH = 2;
    uint256 public constant PLATFORM_FEE_PERCENT = 83; // 8.3% (multiply by 10 for precision)
    uint256 public constant FIRST_PLACE_PERCENT = 917; // 91.7% (winner takes all after platform fee)
    ...
    struct Match {
        ...
        uint8[PLAYERS_PER_MATCH] placements; // 1 = winner, 2 = loser
        bool finalized;
        uint256 prizePool;
        uint256 timestamp;
    }
     ...
     * @param _placements Array of placements (1-2) for each player
     */
     ...
     * Winner takes all (91.7%) after platform fee (8.3%)
     */
    function _distributePrizes(
        ...
        // Remaining pool for distribution (winner takes all)
        uint256 distributionPool = _prizePool - platformFee;
        uint256 winnerPrize = distributionPool; // 100% of remaining pool

        // Find winner and distribute
        for (uint256 i = 0; i < PLAYERS_PER_MATCH; i++) {
            if (_placements[i] == 1) {
                playerBalances[_players[i]] += winnerPrize;
                break; // Only one winner in 1v1
            }
        }
    }
